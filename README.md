...
The AI (DeepSeek/LLM):
It sends the user's question along with the actual text from those specific pages to the AI. This ensures the answer is 100% based on official rules.

The UI (Gradio): 
I chose Gradio for the interface because it's fast to deploy and let me build a custom grade calculator that respects the official "truncation to 2 decimals" rule used by the university.
